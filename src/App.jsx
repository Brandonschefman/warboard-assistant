...
const FONTS = `@import url('https://fonts.googleapis.com/css2?family=Oswald:wght@300;400;600;700&family=Share+Tech+Mono&display=swap');`;
...
- { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .app { font-family:var(–fm); color:var(–text); background:var(–bg);
  display:flex; flex-direction:column; width:100%; height:100vh; overflow:hidden; position:relative; }
  .scanlines { pointer-events:none; position:absolute; inset:0; z-index:9999;
  background:repeating-linear-gradient(0deg,transparent,transparent 2px,rgba(0,0,0,0.05) 2px,rgba(0,0,0,0.05) 4px); }
  .header { display:flex; align-items:center; justify-content:space-between; padding:5px 10px;
  background:var(–surface); border-bottom:1px solid var(–border); flex-shrink:0; gap:6px; flex-wrap:wrap; }
  ...
  .nb { flex:1; padding:7px 4px; background:none; border:none; border-bottom:2px solid transparent;
  color:var(–muted); font-family:var(–fh); font-size:10px; letter-spacing:1px; cursor:pointer; text-transform:uppercase; transition:all 0.15s; }
  ...
  .ctitle { font-family:var(–fh); font-size:11px; letter-spacing:2px; color:var(–accent);
  ...
  input[type=text],input[type=number] { width:100%; background:var(–bg); border:1px solid var(–border);
  color:var(–text); font-family:var(–fm); font-size:13px; padding:5px 8px; border-radius:2px; outline:none; transition:border-color 0.15s; }
  input:focus { border-color:var(–accent); }
  .row { display:flex; gap:8px; margin-bottom:8px; align-items:flex-end; }
  .row > * { flex:1; }
  .btn { background:var(–accent); color:#0d0f0a; border:none; font-family:var(–fh); font-size:11px;
  letter-spacing:2px; padding:8px 12px; cursor:pointer; text-transform:uppercase; font-weight:600;
  border-radius:2px; transition:all 0.15s; white-space:nowrap; width:100%; }
  .btn:hover { opacity:0.88; } .btn:active { transform:scale(0.98); }
  .btn:disabled { opacity:0.35; cursor:not-allowed; }
  .red { background:var(–accent2); color:white; }
  .blue { background:var(–blue); color:white; }
  .bcap { background:var(–capital); color:white; }
  .grn { background:var(–green); color:#0d0f0a; }
  .sm { padding:5px 10px; font-size:9px; }
  ...
  .dopt { flex:1; padding:6px 4px; background:var(–bg); border:1px solid var(–border); border-radius:2px;
  color:var(–muted); font-family:var(–fh); font-size:14px; cursor:pointer; text-align:center; transition:all 0.15s; font-weight:700; }
  ...
  .die { width:28px; height:28px; border-radius:4px; display:flex; align-items:center; justify-content:center;
  font-family:var(–fh); font-size:14px; font-weight:700; border:2px solid; }
  ...
  .olog { background:var(–bg); border:1px solid var(–border); padding:6px 8px; border-radius:2px;
  max-height:200px; overflow-y:auto; font-size:10px; color:var(–muted); margin-top:6px; }
  ...
  .ldie { width:15px; height:15px; border-radius:2px; display:flex; align-items:center; justify-content:center;
  font-size:8px; font-weight:700; border:1px solid; font-family:var(–fh); }
  ...
  .tag.o { border-color:var(–accent); color:var(–accent); }
  ...
  .delbtn { background:none; border:none; color:var(–muted); cursor:pointer; font-size:14px; padding:3px 5px; transition:color 0.15s; }
  .delbtn:hover { color:var(–red); }
  input[type=range] { width:100%; accent-color:var(–accent); }
  .cbadge { display:inline-flex; align-items:center; background:rgba(156,39,176,0.15); border:1px solid var(–capital);
  border-radius:2px; padding:1px 7px; font-size:9px; color:var(–capital); letter-spacing:1px; font-family:var(–fh); }
  .modebtn { background:transparent; border:1px solid var(–border); border-radius:3px; color:var(–muted);
  cursor:pointer; padding:3px 8px; font-family:var(–fh); font-size:9px; letter-spacing:1px; transition:all 0.2s; white-space:nowrap; }
...
const DEF_CONTS = [
{id:1,name:“Europa”,territories:7,bonus:5},
{id:2,name:“Noord-Amerika”,territories:9,bonus:5},
{id:3,name:“Zuid-Amerika”,territories:4,bonus:2},
{id:4,name:“Afrika”,territories:6,bonus:3},
{id:5,name:“Azië”,territories:12,bonus:7},
{id:6,name:“Oceanië”,territories:4,bonus:2},
];
...
const startBattle = () => {
setAtkRem(Number(atkT)); setDefRem(Number(defT));
setRnd(0); setBLog([]); setLiveR(null); setSimRes(null);
};

const rollRound = () => {
const ca = atkRem !== null ? atkRem : Number(atkT);
const cd = defRem !== null ? defRem : Number(defT);
...
```
    {/* LOCK */}
    {locked && (
      <div onClick={()=>setLocked(false)} style={{position:'absolute',inset:0,zIndex:800,
        background:'rgba(13,15,10,0.97)',display:'flex',flexDirection:'column',
        ...
        <div style={{marginTop:12,display:'flex',gap:20,fontSize:11,color:'var(--muted)',fontFamily:'var(--fh)',letterSpacing:1,flexWrap:'wrap',justifyContent:'center'}}>
    ...
    {/* WIN */}
    {hasWon && (
      <div style={{position:'absolute',inset:0,zIndex:600,background:'rgba(13,15,10,0.96)',
    ...
    <div className="header">
      <div style={{display:'flex',alignItems:'center',gap:8}}>
        ...
        <button onClick={()=>setLocked(true)} style={{background:'transparent',border:'1px solid var(--border)',
          borderRadius:3,color:'var(--muted)',cursor:'pointer',padding:'4px 8px',fontSize:14,lineHeight:1}}>🔒</button>
        <button className={`modebtn ${capsMode?'on':''}`} onClick={()=>setCapsMode(m=>!m)}>
          {capsMode ? '👑 CAPITALS AAN' : '⚔ NORMAAL RISK'}
        </button>
      </div>
      <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap',justifyContent:'flex-end'}}>
        ...
        {capsMode && <>
          ...
          <div style={{display:'flex',gap:5,alignItems:'center'}}>
            ...
            <div className="hstat">
              <div className="hval" style={{color:'var(--capital)'}}>{myCaps}<span style={{fontSize:10,color:'var(--muted)'}}>/{totalCaps}</span></div>
              <div className="hlbl">👑 CAPITALS</div>
            </div>
        ...
        <div className="col">
          <div className="card">
            ...
            <div style={{display:'flex',gap:6,alignItems:'center',marginBottom:8}}>
              ...
              <input type="number" min="1" value={myTerr} onChange={e=>setMyTerr(Number(e.target.value)||1)}
                style={{textAlign:'center',fontSize:22,fontFamily:'var(--fh)',fontWeight:700}}/>
            ...
            <div className="rbox">
              ...
              <div className="bdown">
                Basis: <span>{myTerr} ÷ 3 = {baseR}</span><br/>
                Continent: <span>+{contBonus}</span>
                {capsMode && <> &nbsp;|&nbsp; Capitals: <span style={{color:'var(--capital)'}}>+{capBonus}</span></>}
              </div>
            ...
            <div className="card cc">
              ...
              <div style={{display:'flex',gap:6,alignItems:'center',marginBottom:10}}>
                ...
                <div style={{flex:1,textAlign:'center'}}>
                  <div style={{fontFamily:'var(--fh)',fontSize:28,fontWeight:700,color:'var(--capital)',lineHeight:1}}>
                    {myCaps}<span style={{fontSize:16,color:'var(--muted)'}}>/{totalCaps}</span>
                  </div>
              ...
              <div style={{display:'flex',gap:6,justifyContent:'center',marginBottom:10}}>
                {Array.from({length:totalCaps}).map((_,i)=>(
                  <div key={i} onClick={()=>setMyCaps(i<myCaps?i:i+1)} style={{
                    width:28,height:28,borderRadius:'50%',cursor:'pointer',transition:'all 0.2s',
                    background:i<myCaps?'var(--capital)':'var(--bg)',
                    border:`2px solid ${i<myCaps?'var(--capital)':'var(--border)'}`,
              ...
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                ...
                <div style={{fontFamily:'var(--fh)',fontSize:11,color:'var(--capital)',letterSpacing:1}}>BONUS: +{capBonus}</div>
              </div>
              <div className="sdiv"/>
              <div style={{display:'flex',gap:6,alignItems:'center'}}>
                <div style={{fontSize:9,color:'var(--muted)',letterSpacing:1,whiteSpace:'nowrap'}}>CAPITALS IN SPEL:</div>
                <div style={{display:'flex',gap:4}}>
                  {[2,3,4,5,6].map(n=>(
                    <div key={n} onClick={()=>{setTotalCaps(n);setMyCaps(c=>Math.min(c,n));}} style={{
                      width:22,height:22,borderRadius:2,cursor:'pointer',
                      background:totalCaps===n?'var(--capital)':'var(--bg)',
                      border:`1px solid ${totalCaps===n?'var(--capital)':'var(--border)'}`,
                      color:totalCaps===n?'white':'var(--muted)',
                      display:'flex',alignItems:'center',justifyContent:'center',
                      fontFamily:'var(--fh)',fontSize:11,fontWeight:700,transition:'all 0.15s',
                    }}>{n}</div>
                  ))}
                </div>
              </div>
          ...
          <div className="card">
            ...
            {conts.map(c=>(
              <div key={c.id} style={{marginBottom:5}}>
                <button onClick={()=>toggleCont(c.id)} style={{
                  width:'100%',textAlign:'left',cursor:'pointer',transition:'all 0.15s',
                  background:ownedConts.includes(c.id)?'rgba(200,168,75,0.1)':'var(--bg)',
                  border:`1px solid ${ownedConts.includes(c.id)?'var(--accent)':'var(--border)'}`,
                  color:ownedConts.includes(c.id)?'var(--accent)':'var(--muted)',
                  padding:'6px 10px',borderRadius:2,fontFamily:'var(--fm)',fontSize:11,
        ...
        <div className="col">
          <div className={`card ${isCap?'cc':''}`}>
            ...
            <div style={{display:'flex',gap:6,marginBottom:8}}>
              <button style={{flex:1,padding:'5px',background:!isCap?'var(--accent)':'transparent',
                border:`1px solid ${!isCap?'var(--accent)':'var(--border)'}`,color:!isCap?'#0d0f0a':'var(--muted)',
                borderRadius:2,fontFamily:'var(--fh)',fontSize:10,letterSpacing:1,cursor:'pointer'}}
                onClick={()=>setIsCap(false)}>NORMAAL</button>
              <button style={{flex:1,padding:'5px',background:isCap?'var(--capital)':'transparent',
                border:`1px solid ${isCap?'var(--capital)':'var(--border)'}`,color:isCap?'white':'var(--muted)',
                borderRadius:2,fontFamily:'var(--fh)',fontSize:10,letterSpacing:1,cursor:'pointer'}}
                onClick={()=>setIsCap(true)}>👑 KAPITAAL</button>
            </div>
            {isCap && <div style={{background:'rgba(156,39,176,0.1)',border:'1px solid var(--capital)',borderRadius:2,
              padding:'5px 10px',marginBottom:8,fontSize:9,color:'var(--capital)',letterSpacing:1}}>
              KAPITAAL: Verdediger gooit altijd 3 dobbelstenen</div>}
            <div className="row">
            ...
            <div className="dsel">
              {[1,2,3].map(n=><div key={n} className={`dopt ${atkDice===n?'da':''}`} onClick={()=>setAtkDice(n)}>{n}</div>)}
            </div>
            <label style={{marginBottom:4}}>🛡 Verdediger dobbelstenen {isCap&&<span className="cbadge">👑 VAST: 3</span>}</label>
            <div className="dsel" style={{opacity:isCap?0.35:1,pointerEvents:isCap?'none':'auto'}}>
              {[1,2,3].map(n=><div key={n} className={`dopt ${effDef===n?(isCap?'dc':'dd'):''}`} onClick={()=>setDefDice(n)}>{n}</div>)}
            </div>
            ...
            <button className="btn ghost" style={{fontSize:9,padding:'5px',letterSpacing:1}} onClick={startBattle}>↺ NIEUW GEVECHT STARTEN</button>
          </div>
          ...
          {simRes !== null && (
            <div className="card">
              ...
              <div style={{textAlign:'center',marginTop:6}}>
                <span className={`tag ${simRes>=60?'g':simRes>=40?'o':'r'}`}>
                  {simRes>=60?'✓ AANRADEN':simRes>=40?'⚠ TWIJFELACHTIG':'✗ RISKANT'}
                </span>
              </div>
            </div>
          )}
        ...
        <div className="col">
          {liveR && (
            <div className={`card ${isCap?'cc':''}`}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:6}}>
                ...
                <div style={{fontSize:9,color:'var(--muted)',fontFamily:'var(--fh)',letterSpacing:1}}>
                  Overig: <span style={{color:'var(--accent2)'}}>{atkRem}⚔</span> <span style={{color:isCap?'var(--capital)':'var(--blue)'}}>{defRem}🛡</span>
                </div>
              </div>
              <div style={{display:'flex',gap:12,justifyContent:'center',alignItems:'center'}}>
                <div style={{textAlign:'center'}}>
                  <div style={{fontSize:9,color:'var(--muted)',marginBottom:4,letterSpacing:1}}>AANVALLER</div>
                  <div className="drow">{liveR.ad.map((d,i)=><div key={i} className="die da">{d}</div>)}</div>
                </div>
                <div style={{fontFamily:'var(--fh)',fontSize:11,color:'var(--muted)'}}>VS</div>
                <div style={{textAlign:'center'}}>
                  <div style={{fontSize:9,color:isCap?'var(--capital)':'var(--muted)',marginBottom:4,letterSpacing:1}}>{isCap?'👑 KAPITAAL':'VERDEDIGER'}</div>
                  <div className="drow">{liveR.dd.map((d,i)=><div key={i} className={`die ${isCap?'dc':'dd'}`}>{d}</div>)}</div>
                </div>
              </div>
              <div style={{textAlign:'center',marginTop:8,fontSize:11,color:'var(--muted)'}}>
                Aanvaller verliest <span style={{color:'var(--accent2)',fontWeight:'bold'}}>{liveR.aLoss}</span> &nbsp;|&nbsp;
                Verdediger verliest <span style={{color:'var(--green)',fontWeight:'bold'}}>{liveR.dLoss}</span>
              </div>
            </div>
          )}
          ...
          <div className="card">
            ...
            <div style={{fontSize:10,color:'var(--muted)',marginBottom:6,lineHeight:1.6}}>
              Registreer het resultaat — gebiedsteller past automatisch aan:
            </div>
            ...
            <div style={{marginTop:8,textAlign:'center',fontSize:11,fontFamily:'var(--fh)',color:'var(--muted)',letterSpacing:1}}>
              NU: <span style={{color:'var(--accent)',fontSize:15}}>{myTerr}</span> &nbsp;→&nbsp;
              <span style={{color:'var(--green)'}}>{myTerr+1}</span> of <span style={{color:'var(--red)'}}>{Math.max(1,myTerr-1)}</span>
            </div>
          </div>
          ...
          {bLog.length > 0 && (
            <div className="card">
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:6}}>
                ...
                <div style={{fontSize:10,fontFamily:'var(--fh)',letterSpacing:1,display:'flex',gap:8}}>
                  <span style={{color:'var(--accent2)'}}>ATK:{atkRem??Number(atkT)}</span>
                  <span style={{color:isCap?'var(--capital)':'var(--blue)'}}>DEF:{defRem??Number(defT)}</span>
                </div>
              </div>
              <div className="olog">
                ...
                {(()=>{
                  const tA=bLog.reduce((s,e)=>s+e.aLoss,0),tD=bLog.reduce((s,e)=>s+e.dLoss,0);
                  const last=bLog[bLog.length-1],over=last.atkAfter<=1||last.defAfter<=0;
                  return(
                    <div className="lsum">
                      <span style={{color:'var(--muted)'}}>{bLog.length} rondes</span>
                      <span><span style={{color:'var(--accent2)'}}>ATK −{tA}</span><span style={{color:'var(--muted)'}}> | </span><span style={{color:isCap?'var(--capital)':'var(--blue)'}}>DEF −{tD}</span></span>
                      {over&&<span style={{color:last.defAfter<=0?'var(--green)':'var(--red)',fontWeight:'bold'}}>{last.defAfter<=0?'✓ GEWONNEN':'✗ GESTOPT'}</span>}
                    </div>
                  );
                })()}
              </div>
          ...
          <div className="card">
            ...
            {conts.map(c=>(
              <div key={c.id} className="crow">
                ...
                <input type="number" value={c.bonus} style={{flex:1}} placeholder="Bonus" onChange={e=>setConts(p=>p.map(x=>x.id===c.id?{...x,bonus:Number(e.target.value)}:x))}/>
                <button className="delbtn" onClick={()=>delCont(c.id)}>✕</button>
              </div>
            ))}
            <div className="sdiv"/>
            <div style={{fontSize:9,color:'var(--muted)',marginBottom:6,letterSpacing:1}}>NIEUW CONTINENT</div>
            <div className="crow">
        ...
        <div className="col">
          ...
          <div className="card">
            ...
            <div style={{fontSize:10,color:'var(--muted)',lineHeight:2}}>
              ...
              {capsMode&&<>• Capitals: +2 troepen per veroverde capital<br/>• Alle {totalCaps} capitals = GEWONNEN<br/></>}
              • <span className="cbadge">👑 KAPITAAL</span> verdedigt met 3 dobbelstenen<br/>
              • Aanvaller: 1-3 dobbelstenen (kiesbaar)<br/>
